...
// here the condition means that if the req is sent to that definite only he should see the array info, other user's shouldn't see that ! 
  ...
  let handleAccept = (item)=>{
    ...
    set(push(ref(db, "friendlist/")), {
      ...item
      // here we used spread operator to store the data of user, we previously stored all info in user and we'll fetch the data by this method
    }).then(() => {
      ...
      >
       ...
       {friendlist.map((user) => (
            ...
            >
              ...
              <div>
                <h3 className="font-semibold text-lg">{user.sendername}</h3>
                <p className="text-sm opacity-75">{user.senderemail}</p>
              </div>
              <motion.button
                whileTap={{ scale: 0.9 }}
